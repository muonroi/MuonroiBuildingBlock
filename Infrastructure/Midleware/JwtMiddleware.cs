    ...
    Func<IServiceProvider, HttpContext, Task<VerifyToken>> callbackVerifyToken)
{
    ...
    public async Task Invoke(HttpContext context, IServiceProvider serviceProvider)
    {
        ...
        AddHeader(headers, nameof(AuthInfoContext.Language), language.FirstOrDefault() ?? "vi-VN");
        ...
        AddHeader(headers, nameof(AuthInfoContext.CorrelationId), verifyToken.CorrelationId);

        AddHeader(headers, nameof(AuthInfoContext.UserId), verifyToken.UserId);

        AddHeader(headers, nameof(AuthInfoContext.Roles), string.Join(',', verifyToken.Roles));

        AddHeader(headers, nameof(AuthInfoContext.IsAuthenticated), verifyToken.IsAuthenticated.ToString());
